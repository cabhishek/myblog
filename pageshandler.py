...
class PageForm(djangoforms.ModelForm):
    title = forms.CharField(widget=forms.Textarea(attrs={'id':'name', 'rows': 5, 'cols': 50}))
...
class PageHandler(BaseHandler):
  ...
  def post(self, post):
    ...
    if form.is_valid():
        ...
        content = models.PagePost(key_name=page_key, path=post.path, title=post.title, body=post.body)
        content.put()
